    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      if (currentUser) {
        ...
        if (currentUserData) {
          ...
          if (currentUserData.savedThreads && currentUserData.savedThreads.length > 0) {
            try {
              const { getThreadsByIds } = await import("../services/threadService");
    ...
    <div className="min-h-screen bg-gray-50 space-y-6 overflow-y-auto [&::-webkit-scrollbar]:hidden pb-20">
      ...
      <div className="bg-white rounded-xl p-6 flex flex-col md:flex-row items-center gap-6 relative shadow-sm border border-gray-100">
        ...
        <div className="flex-1 text-center md:text-left">
          ...
          <div className="flex justify-center md:justify-start gap-6 mt-5">
            ...
            <Stat label="Saved" value={(savedPosts.length + savedThreads.length) || 0} />
        ...
        <div className="space-y-3">
          {myThreads.length > 0 ? (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-4 text-xs text-gray-500">
                      <span className="flex items-center gap-1">
                        ...
                        {(thread.upvotes?.length || 0) - (thread.downvotes?.length || 0)} votes
                      </span>
                      <span className="flex items-center gap-1">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      <span className="ml-auto">
                        {thread.createdAt?.toDate ? new Date(thread.createdAt.toDate()).toLocaleDateString() : "Recently"}
                      </span>
                    </div>
                  </div>
                </div>
            ...
            <div className="text-center py-16 bg-white rounded-xl border border-dashed border-gray-300">
              ...
              <p className="text-gray-400 text-sm mt-1">Start a discussion to see it here</p>
            </div>
          )}
        </div>
      )}
        ...
        <div>
          ...
          <div className="flex gap-3 mb-6">
            <button
              ...
              className={`px-6 py-2 rounded-xl font-semibold transition-all ${savedFilter === "posts" ? "bg-blue-600 text-white shadow" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`}
            >
              Posts ({savedPosts.length})
            </button>
            <button
              onClick={() => setSavedFilter("threads")}
              className={`px-6 py-2 rounded-xl font-semibold transition-all ${savedFilter === "threads" ? "bg-blue-600 text-white shadow" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`}
            >
              ...
              <div className="grid grid-cols-3 gap-1">
                {savedPosts.map((post) => (
                  <div
                    ...
                    className="aspect-square bg-gray-100 relative group overflow-hidden cursor-pointer"
                  >
                    <img src={post.imageUrl} alt="Saved post" className="w-full h-full object-cover group-hover:opacity-75 transition" />

                    {/* Remove bookmark button */}
                    <button
                      onClick={async (e) => {
                        e.stopPropagation();
                        const { toggleBookmark } = await import("../services/interactionService");
                        try {
                          await toggleBookmark(user.uid, post.id, true);
                          const updated = savedPosts.filter(p => p.id !== post.id);
                          setSavedPosts(updated);
                          updateUser({ savedPosts: user.savedPosts.filter(id => id !== post.id) });
                        } catch (error) {
                ...
                {savedThreads.map((thread) => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0 cursor-pointer" onClick={() => navigate(`/threads/${thread.id}`)}>
                        <h3 className="text-lg font-bold text-gray-900 group-hover:text-blue-600 transition mb-2">
                          {thread.title}
                        </h3>
                        <div
                      ...
                      <button
                        onClick={async () => {
                          ...
                          try {
                            await toggleThreadBookmark(user.uid, thread.id, true);
                            const updated = savedThreads.filter(t => t.id !== thread.id);
                            setSavedThreads(updated);
                            updateUser({ savedThreads: user.savedThreads?.filter(id => id !== thread.id) || [] });
                          } catch (error) {
                            console.error("Error removing from saved:", error);
                          }
                        }}
      ...
      {active === "Settings" && (
        <div className="bg-white rounded-xl p-6 space-y-6 border border-gray-100">
          ...
          <Toggle label="Dark Mode" isOn={theme === 'dark'} onToggle={toggleTheme} />
